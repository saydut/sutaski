...
class AdminService:
    ...
    def get_all_data(self):
        ...
        try:
            sirketler = supabase.table('sirketler').select('*').execute().data
            kullanicilar = supabase.table('kullanicilar').select('*, sirketler(sirket_adi)').execute().data
    ...
    def update_license(self, sirket_id: int, yeni_tarih: str):
        ...
        try:
            ...
            supabase.table('sirketler').update({'lisans_bitis_tarihi': tarih_degeri}).eq('id', sirket_id).execute()
        except Exception as e:
    ...
    def _get_setting(self, ayar_adi: str, varsayilan: str) -> str:
        ...
        response = supabase.table('ayarlar').select('ayar_degeri').eq('ayar_adi', ayar_adi).limit(1).single().execute()
    ...
    def get_all_version_notes(self):
        ...
        return supabase.table('surum_notlari').select('*').order('yayin_tarihi', desc=True).order('id', desc=True).execute().data
    ...
    def add_version_note(self, data: dict):
        ...
        supabase.table('surum_notlari').insert(data).execute()

    def update_version_note(self, id: int, data: dict):
        if not all([data.get('surum_no'), data.get('yayin_tarihi'), data.get('notlar')]):
            raise ValueError("Tüm alanlar zorunludur.")
        response = supabase.table('surum_notlari').update(data).eq('id', id).execute()
        if not response.data:
            raise ValueError("Güncellenecek sürüm notu bulunamadı.")
            
    def delete_version_note(self, id: int):
        response = supabase.table('surum_notlari').delete().eq('id', id).execute()
        if not response.data:

...
CREATE TABLE public.toplayici_tanker_atama (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    firma_id INTEGER NOT NULL REFERENCES public.sirketler(id) ON DELETE CASCADE,
    -- Toplayıcının 'kullanicilar' tablosundaki ID'si (BIGINT olduğunu varsayıyoruz)
...
-- HATA DÜZELTMESİ: 'sut_kayitlari' -> 'sut_girdileri'
...
CREATE TABLE public.sut_satis_islemleri (
    ...
    miktar_kg NUMERIC(10, 2) NOT NULL, -- (veya litre, maliyet hesabına göre kg daha mantıklı olabilir)
...
-- (Öncekini silip yenisini yaratıyoruz, Mesaj 46/47'deki güncel hali)
